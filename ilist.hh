...
struct ilist_entry : private detail::ilist_node {
    ...
    ~ilist_entry() { if (prev_.both_) unlink(); }
...
public:
	...
	detail::ilist_node *vnew = &(to_insert->*EP);
	ILIST_ASSERT(!vnew->prev_.both_); // to_insert must not be in list
	detail::ilist_node *vpos = pos ? &(pos->*EP) : &head_;
    ...
    static void remove(value_type *v) { (v->*EP).unlink(); }
    ...
    static value_type *next(value_type *v) { return to_value((v->*EP).next_); }
    static const value_type *next(const value_type *v) {
	return to_value((v->*EP).next_);
    }
    static value_type *prev(value_type *v) { return to_value((v->*EP).prev_); }
    static const value_type *prev(const value_type *v) {
	return to_value((v->*EP).prev_);
    }
