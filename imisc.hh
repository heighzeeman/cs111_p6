...
template<typename S, typename F> inline std::ptrdiff_t
field_offset(F S::*fp)
{
...
template<typename S, typename F> inline S *
struct_from_field(F S::*fp, F *f)
{
    return reinterpret_cast<S *>(reinterpret_cast<char *>(f) -
				 field_offset(fp));
}

template<typename S, typename F> inline const S *
struct_from_field(F S::*fp, const F *f)
{
...
struct fake_iterator {
    ...
    fake_iterator &operator++() { v_ = T::next(v_); return *this; }
    V *operator++(int) {
	V *v{v_};
	++*this;
	return v;
    }
    fake_iterator &operator--() { v_ = T::prev(v_); return *this; }
    V *operator--(int) {
	V *v{v_};
	--*this;
	return v;
