...
template<auto Field> using struct_of_field =
    typename struct_of_field_helper<Field>::type;
...
template<auto Field> using type_of_field =
    typename type_of_field_helper<Field>::type;
...
//! functions.
template<auto Link, typename Compare = std::less<detail::struct_of_field<Link>>>
    class itree_core;
...
template<auto KeyPtr,
	 itree_entry detail::struct_of_field<KeyPtr>::*Link,
	 typename Compare = std::less<detail::type_of_field<KeyPtr>>>
class itree;
...
//! that one of functions can work on all types.
class itree_base {
...
public:
    ...
    ~itree_entry() { if (is_linked()) unlink(); }
    ...
    template<auto Link, typename Compare> friend class itree_core;
};

template<typename V, itree_entry V::*EP, typename C>
class itree_core<EP, C> : detail::itree_base {
	...
	return n ? struct_from_field(EP, static_cast<itree_entry *>(n)) :
	    nullptr;
    }
    static const V *to_value(const detail::itree_node *n) {
	return n ?
	    struct_from_field(EP, static_cast<const itree_entry *>(n)) :
	    nullptr;
    }
...
public:
	...
	return to_value(itree_successor(&(v->*EP)));
    }
	...
	return to_value(itree_predecessor(&(v->*EP)));
    }
	...
	return static_cast<itree_core *>((v->*EP).base());
    }
    ...
    //! within the range.  If you don't have duplicate values in your
    //! tree, this function will return the same element as
    //! lower_bound.
    //!
    //! If you are looping between \c lower_bound() and \c
    //! upper_bound_prev(), you must process both elements if they are
    //! distinct.  For that reason, \c upper_bound() is a more useful
    //! loop sentinel \c upper_bound_prev().
    //!
    //! The reason to supply \c upper_bound_prev() in addition to \c
    //! upper_bound() is that you can't walk the \c end() iterator
    //! backwards.  Hence, if you need the last value before some
    //! maximum you have to use \c upper_bound_prev() rather than
    //! taking the \c prev(upper_bound()).
    template<typename K, typename CC = C>
...
namespace detail {
template<auto KeyPtr, typename Compare = std::less<type_of_field<KeyPtr>>>
struct field_compare;
template<typename K, typename V, K V::*KP, typename C>
struct field_compare<KP, C> {
    using value_type = V;
    using key_type = K;
    static constexpr key_type value_type::*field_ptr = KP;
    using compare_type = C;

    bool operator()(const value_type &v1, const value_type &v2) const {
	return compare_type()(v1.*field_ptr, v2.*field_ptr);
    }
    bool operator()(const key_type &k, const value_type &v) const {
	return compare_type()(k, v.*field_ptr);
    }
    bool operator()(const value_type &v, const key_type &k) const {
	return compare_type()(v.*field_ptr, k);
    }
};
} // namespace detail

template<typename K, typename V, K V::*KP, itree_entry V::*EP, typename C>
class itree<KP, EP, C>
    : public itree_core<EP, detail::field_compare<KP, C>>
{
public:
    using core_type = itree_core<EP, detail::field_compare<KP, C>>;
    using key_type = K;
    using typename core_type::value_type;
    using typename core_type::compare_type;
    ...
    value_type *operator[](const K &k) { return this->find(k, compare_type()); }
...
//! }
//! \endcode
template<typename C, typename F, typename...Rest> inline bool
multiless(const C &a, const C &b, F C::*f, Rest...rest)
{
    return a.*f < b.*f || (!(b.*f < a.*f) && multiless(a, b, rest...));
}
template<typename C> inline bool
multiless(const C &a, const C &b)
{
    return false;
}
